...
def _extract_first_json_object(text: str) -> str:
    """ดึง JSON object ก้อนแรกจากข้อความที่ AI ตอบกลับ (กันหลุด Markdown/คำอธิบาย)"""
    if not text:
        raise ValueError("Empty model response")

    t = text.strip()

    # ถ้ามี code fence ให้พยายามตัดออก
    if "```" in t:
        # เลือกช่วงที่มี { }
...
def _normalize_scores(scores_raw):
    """ทำให้ scores เป็น list จำนวน 17 ค่า (ข้อ 4-20) และ clamp 0-5"""
...
def save_to_csv(staff_name, customer_name, scores, total, passed):
    """บันทึกคะแนนลง CSV (เขียนทุกครั้ง)"""
...
def get_audio_base64(text, voice_config):
    """Google TTS -> base64 (MP3)"""
    if not TTS_API_KEY:
        return None

    # ตัดข้อความในวงเล็บ/ชื่อผู้พูดออก ลดเสียงอ่านแปลกๆ
...
# ลด noise ใน log บน Vercel (ไม่บังคับ)
...
def chat():
    try:
        ...
        cust = CUSTOMERS.get(lvl)
        if not cust:
            return jsonify({"reply": "ขอโทษครับ ไม่พบระดับลูกค้า", "audio": None}), 400
...
2) ให้เหตุผลโดยอ้างอิงข้อความจริงจากบทสนทนา (evidence) อย่างน้อย 1 ชิ้นต่อข้อ
   - evidence ควรเป็นข้อความที่ขึ้นต้นด้วย 'พนักงาน:' เพื่อยืนยันหลักฐาน
...
"""
        ...
        if isinstance(fb, list) and fb:
            fb_map = {}
            for x in fb:
                if isinstance(x, dict) and str(x.get("item", "")).isdigit():
                    fb_map[int(x.get("item"))] = x

            for idx, (item_no, title) in enumerate(QC_ITEMS):
                obj = fb_map.get(item_no, {})
                # ถ้า AI ส่ง score รายข้อมา ให้ใช้ แต่ถ้าไม่ส่งให้ใช้จาก array scores
                score_item = _to_int_score(obj.get("score", scores[idx]), scores[idx])
...
out = Path('/mnt/data/index_FULL_ready.py')
