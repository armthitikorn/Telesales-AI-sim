...
def evaluate():
    try:
        ...
        # ปรับปรุง Prompt ให้มีหัวข้อการประเมิน 17 ข้อที่ชัดเจนและสมจริง
        eval_prompt = f"""ในฐานะ QA (Quality Assurance) ผู้เชี่ยวชาญด้านการขายประกันผ่านโทรศัพท์
        โปรดวิเคราะห์และประเมินประวัติการสนทนานี้อย่างละเอียด โดยให้คะแนนตามเกณฑ์ 17 ข้อต่อไปนี้ (คะแนนเต็มข้อละ 5 คะแนน)
        หากไม่มีการพูดถึง หรือทำได้ไม่ครบถ้วนให้หักคะแนนตามความเหมาะสม
        
        เกณฑ์การประเมิน 17 ข้อ:
        1. การแนะนำตัว แจ้งชื่อ-นามสกุล และชื่อบริษัทอย่างชัดเจน
        2. การแจ้งวัตถุประสงค์ในการโทร และขออนุญาตผู้มุ่งหวัง
        3. การสร้างความสนใจ (Hook) หรือวิเคราะห์ความต้องการเบื้องต้น
        4. การสอบถามข้อมูลและค้นหาความต้องการของลูกค้า (Fact Finding)
        5. การนำเสนอแบบประกันที่สอดคล้องกับความต้องการของลูกค้า
        6. การอธิบายผลประโยชน์และความคุ้มครองที่ชัดเจน เข้าใจง่าย
        7. การแจ้งเงื่อนไข ข้อยกเว้น หรือข้อจำกัดที่สำคัญของกรมธรรม์
        8. การแจ้งเบี้ยประกันและระยะเวลาชำระเบี้ยที่ถูกต้อง
        9. การตอบข้อซักถามและจัดการข้อโต้แย้ง (Handling Objections) ได้อย่างมืออาชีพ
        10. การไม่ใช้คำพูดหลอกลวง บังคับ คาดคั้น หรือกดดันลูกค้าจนเกินไป
        11. การแจ้งสิทธิขอยกเลิกกรมธรรม์ (Free look period)
        12. การแจ้งความจำเป็นในการแถลงข้อมูลสุขภาพตามความเป็นจริง (ถ้ามี)
        13. การนำเสนอช่องทางการชำระเงิน
        14. การสรุปข้อมูลสำคัญทั้งหมดให้ลูกค้าฟังอีกครั้งก่อนปิดการขาย
        15. ความพยายามและทักษะในการปิดการขาย (Closing)
        16. การใช้น้ำเสียง สุภาพ ชัดเจน และมีจังหวะการพูดที่น่าฟัง
        17. การกล่าวลาและขอบคุณลูกค้าอย่างเหมาะสม
        
        ประวัติการสนทนา: 
        {history}
        
        ตอบกลับเป็น JSON Format เท่านั้น ตามโครงสร้างนี้ ห้ามมีข้อความอื่นปน:
        {{
            "scores": [คะแนนข้อ1ถึง17 เรียงตามลำดับ เป็นตัวเลข 0 ถึง 5],
            "strengths": "สรุปจุดแข็งของพนักงาน (1-2 ประโยค)",
            "weaknesses": "สรุปจุดที่ควรปรับปรุง (1-2 ประโยค)",
            "improvements": "คำแนะนำเพิ่มเติมเพื่อให้ปิดการขายได้ดีขึ้นตามหลัก คปภ."
        }}"""
    ...
    <script>
        ...
        async function showEvaluation() {
            ...
            try {
                ...
                // บังคับกางตารางคะแนน 17 ข้อเสมอ ไม่ว่า Data จะกลับมาครบหรือไม่
...
def evaluate():
    try:
        ...
        # ปรับแก้ Prompt ให้ละเอียดและบังคับรูปแบบ JSON
        ...
        # กรองเอาเฉพาะตัวแปร JSON ออกมาให้ชัวร์ที่สุด
        import re
        ...
        # ถ้าคะแนนมาไม่ครบ 17 ข้อ ให้เติม 0 ให้เต็ม
        while len(scores) < 17:
            scores.append(0)
        scores = scores[:17]
        ...
        return jsonify({
            ...
            "total": 0,
            "passed": False,
            "strengths": "ไม่พบข้อมูล",
            "weaknesses": "ระบบไม่สามารถดึงข้อมูล JSON จาก AI ได้อย่างสมบูรณ์",
            "improvements": "โปรดตรวจสอบประวัติการสนทนาหรือการเชื่อมต่ออีกครั้ง"
        })
